...
-- ============================================
-- 核心修复：先删除所有外键约束（避免删表时依赖索引）
-- ============================================
-- 先解除所有外键依赖，再删表
ALTER TABLE `alert` DROP FOREIGN KEY IF EXISTS `fk_alert_monitor_id`;
ALTER TABLE `alert` DROP FOREIGN KEY IF EXISTS `fk_alert_user_id`;
ALTER TABLE `monitor_history` DROP FOREIGN KEY IF EXISTS `fk_monitor_history_monitor_id`;
ALTER TABLE `monitor` DROP FOREIGN KEY IF EXISTS `fk_monitor_user_id`;
ALTER TABLE `alert_config` DROP FOREIGN KEY IF EXISTS `fk_alert_config_user_id`;
...
) ENGINE=InnoDB CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci COMMENT='监控项表';
...
CREATE TABLE `monitor_history` (
                                   ...
                                   `error_msg` TEXT DEFAULT NULL COMMENT '错误信息',  -- 从 VARCHAR(500) 改为 TEXT
...
) ENGINE=InnoDB CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci COMMENT='监控历史表';
...
) ENGINE=InnoDB CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci COMMENT='告警配置表';
...
) ENGINE=InnoDB CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci COMMENT='告警记录表'; -- 修复COLLATE拼写错误
...
-- ============================================
-- 创建索引优化查询性能（兼容MySQL 5.7+）
-- ============================================

-- 先检查索引是否存在，不存在再创建（兼容低版本MySQL）
-- 监控项表索引
SELECT COUNT(*) INTO @cnt FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'monitor' AND INDEX_NAME = 'idx_monitor_user_status';
SET @sql = IF(@cnt = 0, 'CREATE INDEX `idx_monitor_user_status` ON `monitor`(`user_id`, `status`)', 'SELECT ''Index already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 告警记录表索引
SELECT COUNT(*) INTO @cnt FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'alert' AND INDEX_NAME = 'idx_alert_user_status_time';
SET @sql = IF(@cnt = 0, 'CREATE INDEX `idx_alert_user_status_time` ON `alert`(`user_id`, `status`, `created_at`)', 'SELECT ''Index already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 监控历史表索引
SELECT COUNT(*) INTO @cnt FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'monitor_history' AND INDEX_NAME = 'idx_monitor_history_monitor_time';
SET @sql = IF(@cnt = 0, 'CREATE INDEX `idx_monitor_history_monitor_time` ON `monitor_history`(`monitor_id`, `monitor_time`)', 'SELECT ''Index already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;